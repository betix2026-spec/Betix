...
@router.get("/_diag/find-team-match")
async def _diag_find_team_match(q: str, x_internal_secret: Optional[str] = Header(None)):
    """
    TEMPORARY — find a specific match by team name substring, checking
    both analytics.football_matches and public.matches, and the teams
    table for a name match at all (to distinguish "never ingested" from
    "ingested but wrong status" from "ingested but never synced to
    public"). q is matched case-insensitively against team names.
    """
    _check_internal_secret(x_internal_secret)
    settings = get_settings()
    ...
    teams = await asyncio.to_thread(
        ...
        "teams",
        f"select=id,name,sport&name=ilike.*{q}*",
    )
    ...
    if team_ids:
        ...
        analytics_rows = await asyncio.to_thread(
            db_analytics.select_raw,
            "football_matches",
            f"select=*&or=(home_team_id.in.({id_list}),away_team_id.in.({id_list}))&order=date_time.desc&limit=10",
        )

    public_rows = await asyncio.to_thread(
        db_public.select_raw,
        "matches",
        f"select=*&home_team->>name=ilike.*{q}*&limit=10",
    )
    public_rows_away = await asyncio.to_thread(
        db_public.select_raw,
        "matches",
        f"select=*&away_team->>name=ilike.*{q}*&limit=10",
    )

    return {
        "matched_teams": teams,
        "analytics_football_matches": analytics_rows,
        "public_matches_as_home": public_rows,
        "public_matches_as_away": public_rows_away,
    }
